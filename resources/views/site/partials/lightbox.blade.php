    ...
    <div x-show="open" x-transition
        ...
        <div class="bg-transparent rounded shadow-lg overflow-hidden">
                <div class="flex items-center justify-between p-2">
                    ...
                    <div class="text-white"> <span x-text="index + 1"></span> / <span x-text="groupItems.length"></span> </div>
            ...
            <div class="bg-black flex items-center justify-center lightbox-media" style="min-height:300px;">
                <template x-if="groupItems.length">
                    <div class="w-full h-full flex items-center justify-center">
                        <template x-if="groupItems[index].type === 'video'">
                            <video x-bind:src="groupItems[index].url" controls playsinline class="max-h-[80vh] max-w-full" x-ref="video">
                                Your browser does not support HTML5 video.
                            </video>
                        </template>
                        <template x-if="groupItems[index].type !== 'video'">
                            <img x-bind:src="groupItems[index].url" alt="" class="max-h-[80vh] max-w-full object-contain">
                        </template>
                    </div>
                </template>
            </div>
